...
export const SoundSelectorModal: React.FC<Props> = ({
  isOpen,
  onClose,
  onSoundChange,
        ...
        <div className="px-4 sm:px-6 py-3.5 bg-slate-50 dark:bg-slate-800/80 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between gap-3 shrink-0">
          <div className="flex items-center gap-2">
            <p className="text-[10px] sm:text-[11px] text-slate-500 dark:text-slate-400 flex items-center gap-1.5 leading-tight">
              ...
              <span className="hidden sm:inline">Settings and custom sounds saved locally.</span>
            </p>
            {onOpenAndroidAppModal && (
              <button
                onClick={() => {
                ...
                className="px-2.5 py-1 rounded-lg text-[10px] font-bold bg-emerald-100 dark:bg-emerald-950 text-emerald-800 dark:text-emerald-300 hover:bg-emerald-200 transition-colors flex items-center gap-1"
              >
                <Smartphone className="w-3 h-3 text-emerald-600 dark:text-emerald-400" />
                <span>Android Lock Screen Setup</span>
              </button>
            )}
          </div>
          <button
